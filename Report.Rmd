...
```
The upregulated genes are strongly associated with processes involved in cell division, chromosome organization, and mitosis. Enrichment related to mitosis and chromosome segregation implies that the upregulated genes are driving cell proliferation, this could lead to rapid tumor cells proliferation. 
...
```
The downregulation of these molecular functions is consistent with: ECM remodeling,
loss of adhesion, altered signaling, increased metastasis, tumor progression, microenvironment alterations.
This functional downregulation aligns with hallmarks of cancer such as tissue invasion, metastasis, and microenvironmental deregulation.


**WikiPathways (WP) analysis**, is a type of pathway enrichment  analysis that uses pathways from the WikiPathways database. 
...
Using KEGG enrichment analysis it were visualized the top ten enriched pathway from upregulated genes.
...
Now we focus on identifying **transcription factors (TFs) enriched** in the promoter sequences of up-regulated genes using motif enrichment analysis. Motif enrichment analysis determines whether specific TF binding motifs are statistically over-represented (enriched) in promoters of up-regulated genes. Over-representation indicates that certain TFs may play regulatory roles in the observed changes in gene expression.
The used tool `PWMEnrich` use Position Weight Matrices (PWMs) to scan sequences and calculate enrichment scores. PWMs represent the likelihood of observing a specific nucleotide at each position of a binding site motif. Scoring and enrichment tests provide statistical significance values (p-values) for over-represented motifs.
...
Secondly, `PWMLogn.hg19.MotifDb.Hsap` loads a precomputed background model for motif enrichment using TF motifs from `MotifDb`.
`motifEnrichment()` scans the promoter sequences (`seq`) using the background model and calculates motif enrichment scores using "affinity" scoring.
```{r MES}
...
## 7. Motif analyisi for the top enriched TFs
...
## 8. Pattern Matching
The following code identifies specific up-regulated genes whose promoter regions exhibit significant binding scores for transcription factor (TF) motifs. `MotifScores` computes the binding scores of the provided transcription factor motifs (PWMs) for a set of promoter input sequences (`seq`). The output `scores` will be a matrix where each row corresponds to a DNA sequence, and each column represents a PWM. Each value indicates whether the score for a motif exceeds its threshold. `genes_id` will only contain a subset of `up_DEGs` with significant TF motif binding in their promoter regions.
```{r patMatch}
...
library(biomaRt)
```
Firstly, were extracted the  unique names of up-regulated DEGs and save them to a tab-separated file (`PPI_up_DEGs.txt`) for input into STRING.
Secondly, were load STRING interaction data (`string_interactions.tsv`) containing PPI relationships between genes or proteins.
Lastly, the previously exported DEG list was imported for integration with the interaction data.
```{r}
