...
```
The upregulated genes are strongly associated with processes involved in cell division, chromosome organization, and mitosis. Enrichment related to mitosis and chromosome segregation implies that the upregulated genes are driving cell proliferation, this could lead to rapid tumor cellsproliferation. 
...
Now we focus on identifying **transcription factors (TFs) enriched** in the promoter sequences of up-regulated genes using motif enrichment analysis. Motif enrichment analysis determines whether specific TF binding motifs are statistically overrepresented (enriched) in promoters of up-regulated genes. Overrepresentation indicates that certain TFs may play regulatory roles in the observed changes in gene expression.
The used tool `PWMEnrich` use Position Weight Matrices (PWMs) to scan sequences and calculate enrichment scores. PWMs represent the likelihood of observing a specific nucleotide at each position of a binding site motif. Scoring and enrichment tests provide statistical significance values (p-values) for overrepresented motifs.
